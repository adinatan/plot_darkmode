...
for n=1:numel(axes_ind)
    ...
    h(axes_ind(n)).GridColor =  adjust_color(h(axes_ind(n)).GridColor,tcd);
    h(axes_ind(n)).MinorGridColor =  adjust_color(h(axes_ind(n)).MinorGridColor,tcd);
    %h(axes_ind(n)).Subtitle.Color = textcolor;
...
if ~isempty(ha)
    for n=1:numel(ha)
        ...
        for m=1:numel(textboxshape_ind)
            ...
            hAnnotChildren(textboxshape_ind(m)).EdgeColor  =  textcolor;
        end
